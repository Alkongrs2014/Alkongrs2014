...
/* =====================================================================
   مستويات التتبّع — النسبة من سعر المسح المثبَّت، لا من دخول الخطة.

   وهي **غير أهداف الخطة**: الأهداف مقاوماتٌ حقيقية تُقاس من دخولٍ قد
   يكون ارتداداً لم يُلمس، وهذه تقيس ما تحرّكه السهمُ منذ رأيتَه. ولذلك
   يمكن أن تُبلَغ الثلاثة وخطةُ الرمز ما زالت `wait` — قِيس: ‎19‎ من ‎37‎
   سجلاً بلغ المستويات الثلاثة ولم تُفعَّل خطته قط.

   تُعرَّف هنا لأن المتصفّح يعرضها (`renderTrackedLevels`) والخادم يحكم
   بها على انتهاء الفرصة — ورقمان مختلفان في الموضعين يجعلان الشاشة تقول
   «بلغت الثلاثة» والسجلَّ يبقيها نشطة، وهو بالضبط ما بلّغ عنه المستخدم
   في `NEAR-USD`. */
var TRACK_MARKS = [1, 2, 5];

/* =====================================================================
   الفرصة «استُهلكت» — بلغت كلّ مستوياتها المعروضة.

   ولا تعني «أُغلق السجلّ»: الخطة تبقى تُقاس إلى نهايتها فلا يُبتر إحصاء
   الأهداف. تعني أن هذا التحليل **لم يعد الفرصةَ الحالية** — فلا يثبّت
   عمرها ولا سعرَها، ولا يمنع تحليلاً جديداً للشركة نفسها.

   و`mfe` أقصى ما بلغه **باتجاه الإشارة** (الهابطة تربح بالهبوط)، فالمقارنة
   موقَّعة أصلاً ولا تحتاج فرعاً ثانياً. */
function oppSpent(sig) {
  if (!sig) return false;
  var top = TRACK_MARKS[TRACK_MARKS.length - 1];
  return Number.isFinite(sig.mfe) && sig.mfe >= top;
}

if (typeof module !== "undefined" && module.exports) {
  module.exports = { unpackK, pivotBar, levelsFrom, planDirOf, planFrom, planPair, validatePlan,
                     TRACK_MARKS, oppSpent };
}
