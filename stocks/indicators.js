...
/* =====================================================================
   الشمعة الجارية — تمييزها ثم إسقاطها، بتعريفٍ **واحد** يقرؤه الخادم
   والمتصفح وماسح الاستراتيجيات معاً.

   لماذا هنا لا في كلّ مستهلك: «ما هي الشمعة المغلقة الأخيرة؟» سؤالٌ
   يطرحه ثلاثة — `fetch-market` حين يحسب `an`، و`closedRec` حين يبني
   سياق التأكيد، وأيُّ فحصٍ يتحقّق من الثبات. وثلاثُ إجاباتٍ متقاربة
   أسوأ من إجابةٍ واحدة: تتباعد بأول تعديل فيصير المؤكَّد في مكانٍ غيرَه
   في مكان، بلا خطأٍ ولا أثر.

   ---------------------------------------------------------------------
   **الفريمات اللحظية**: طولُ الشمعة ثابتٌ معلوم، فالشمعة جاريةٌ ما دام
   `t + طولها > now`. دقيقٌ بلا تقويم ولا منطقة زمنية ولا حالة جلسة.

   **الفريم اليومي**: لا طول ثابت له — الجلسة ‎6.5‎ ساعات لا ‎24‎،
   فـ`t + 86400000` يعطي جواباً خاطئاً دائماً (شمعة أمس مختومة ‎13:30‎
   تُقرأ «جارية» حتى ‎13:30‎ اليوم، وهي مغلقة منذ ‎20:00‎ أمس). والمقياس
   الصحيح **اليوم نفسه**: الشمعة اليومية جاريةٌ إن كان تاريخُها تاريخَ
   اليوم. وتوقيت UTC يكفي تقويماً — جلسة نيويورك تفتح ‎13:30‎ وتغلق
   ‎20:00‎ UTC فطرفاها في نفس التاريخ دائماً، وشمعة الكريبتو مختومة
   ‎00:00‎ UTC فتاريخها يومها. وهو نفس المنطق الذي تقوم عليه `pivotBar`
   في `plan.js`، ولنفس السبب بالحرف.

   ---------------------------------------------------------------------
   **ولماذا لا تُحذف الأخيرة دائماً بلا سؤال**: ذلك يُسقط شمعةً **مغلقة**
   حين يكون السوق مغلقاً — فتُقرأ الأسهم الأمريكية مساءً على شمعة أمس
   اليومية، أي عمىً كاملٌ عن جلسةٍ انتهت. والحذفُ المشروط يعطي الثبات
   نفسه (المعيار يتبدّل عند إغلاق شمعةٍ لا قبله) بلا هذا الثمن.
   ===================================================================== */
var BAR_MS = { "1m": 6e4, "5m": 3e5, "15m": 9e5, "30m": 18e5,
               "1h": 36e5, "2h": 72e5, "4h": 144e5 };

/* ختمُ الشمعة بالملّي — على الصيغة المحزومة (الزمن بالثواني) كما على
   المفكوكة. المستهلكون يمرّرون الاثنتين، وفرعٌ عند كلٍّ منهم يعني
   نسخةً من القرار تتباعد. */
function barTime(x) {
  if (!x) return null;
  var t = Array.isArray(x) ? x[0] * 1000 : x.t;
  return Number.isFinite(t) ? t : null;
}

/* =====================================================================
   أهذه الشمعة قيد التكوّن؟

   شرطان لا شرط، والثاني كلّف تجميدَ لقطة الفرص ساعةً و‎46‎ دقيقة:

   ١) **لم يمضِ طولُها** — `t + طولها > now`.
   ٢) **ختمُها على شبكة فريمها** — `t % طولها === 0`.

   والثاني ليس تزيّداً: ياهو يُرفق بالسلسلة الممتدّة **طبعةً جزئية
   بختمِ الدقيقة الجارية**. قِيس على `AAPL` يوم ‎2026-09-21‎:
   `tfx["15m"]` تنتهي بـ‎08:45 · 09:00 · 09:15 · **09:16**‎ — والأخيرة
   ليست شمعةَ ربع ساعة أصلاً بل لقطةُ سعرٍ بختمٍ خارج الشبكة.

   وهي من عائلة «ياهو يُرفق مطبعةَ الإغلاق بختم ‎16:00‎ بالضبط»
   الموثّقة — نفس المصدر ونفس الشكل، مطبَّقاً على الجلسة الممتدّة.

   واليوميُّ بلا طولٍ ثابت (الجلسة ‎6.5‎ ساعات لا ‎24‎)، فمعيارُه
   التاريخ نفسه كما كان. */
...
/* =====================================================================
   السلسلة بلا شمعاتها غير المغلقة — **كلُّها لا آخرُها**.

   كانت تحذف واحدةً (`slice(0, -1)`)، وهو صحيحٌ متى كانت السلسلة على
   شبكةٍ منتظمة: لا تكون فيها إلا الأخيرة قيد التكوّن. والافتراض يسقط
   مع الطبعة الجزئية أعلاه: تُحذف ‎09:16‎ فتبقى ‎09:15‎ — **وهي قيد
   التكوّن كذلك** — فتُقرأ «مؤكَّدة» ويُحسب عليها كلُّ شيء.

   وأثرُه لم يكن رقماً خاطئاً بل **تجميداً كاملاً**: صار `confBar`
   شمعةً كاملةً أمام `max(cbar)`، فبوّابةُ لقطة الفرص لا يتساوى لها
   مصدران أبداً، فتوقّفت اللقطة عند ‎07:30Z‎ وكلُّ محاولةٍ تُسجَّل
   «المصدران على شمعتين».

   والحلقة تتوقّف عند أوّل شمعةٍ مغلقة: ما قبلها مغلقٌ بالضرورة على
   أيّ سلسلةٍ مرتَّبة زمنياً. وتُعاد السلسلة **كما هي** إن كانت
