...
/* السلسلة بلا شمعتها الجارية. تُعاد **كما هي** حين تكون الأخيرة مغلقة
   — فلا إزاحةَ بمقدار شمعة في أيّ موضع، وتقرأ البوابات `k[n-1]`
   و`k[n-2]` بنفس دلالاتها بلا تعديل حرفٍ واحد فيها. */
function closedBars(k, tf, now) {
  if (!Array.isArray(k) || k.length < 2) return k || [];
  return isLiveBar(tf, barTime(k[k.length - 1]), now) ? k.slice(0, -1) : k;
}
