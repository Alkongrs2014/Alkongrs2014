...
/* =====================================================================
   ما الذي تضمنه المنطقة الميتة فعلاً؟ **الانقلاب الكامل** لا كلَّ خطوة.

   الصيغة الأولى لهذا الفحص رفضت أيَّ بوابةٍ تنتقل من ‎0‎ إلى ‎±1‎ —
   وهي تخلط صفرين مختلفين:

     · صفرُ `cmp(a, b, atr)` = «طرفاها متقاربان فلا تصوّت» — وهو ما
       بُنيت له المنطقة الميتة، وخروجُه بإزاحةٍ دونها خطأ.
     · صفرُ بوابةٍ ثلاثية الدرجات بحدودٍ **مقصودة** — مثل `vwDist`
       (‎x ≤ 1‎ قريب · ‎x ≤ 2‎ متوسط · فوقها بعيد). صفرُها «مسافةٌ
       متوسطة» لا «حائر»، وعبورُ حدِّ الاثنين سلوكٌ صحيح.

   والفحص كان يمرّ لأن السعر لم يكن يقع على حدٍّ من النوع الثاني، لا
   لأن الحدود محروسة — نفس عائلة «حارسٌ ينجح بالصدفة» التي ظهرت في
   `orb`. فالثابت الذي يجب أن يصمد دائماً هو **ألّا تنقلب بوابةٌ من
   ‎+1‎ إلى ‎−1‎ أو العكس** بإزاحةٍ أصغر من المنطقة الميتة: ذاك تبدُّلُ
   رأيٍ كامل من حركةٍ لا يراها أحد، وهو العلّة التي كلّفت المشروع
   ‎23.5‎ نقطة على الفريم الواحد.
   ===================================================================== */
t("المنطقة الميتة تعمل على سياقٍ حقيقي", () => {
  ...
  for (const sgn of [1, -1]) {
    const c2 = Object.assign({}, c, { px: c.px + nudge * sgn });
    for (const st of S.STRATEGIES) {
      ...
      for (const g of (r2.g || [])) {
        const before = g1[g[0]], after = g[2];
        if (before === undefined || before === null || after === null) continue;
        seen++;
        if (before * after === -1)
          throw new Error(`${st.id}/${g[0]} انقلبت ${before} → ${after} بإزاحةٍ داخل المنطقة الميتة`);
      }
    }
  }
  console.log(`      (${seen} قراءة بوابة على الجهتين)`);
});
...
/* =====================================================================
   تثبيتُ الفريم **خيارٌ محايد** — والمسار بلا تثبيت لا يتحرّك بحرف.

   `tfPin` مسّ خمسَ دوالِّ اختيارٍ (`sqTf`/`iTf15`/`divTf`/`moSlow`/
   `pbTf`) وأعاد كتابة فريمَي `momo` و`pbTrend` في عشرين موضعاً. وأيُّ
   انزلاقٍ في المسار الافتراضي يغيّر كلَّ صفٍّ في الماسح وكلَّ سجلٍّ
   يُكتب — **بلا خطأ ولا استثناء، فقط أرقامٌ أخرى**.

   **وأوّل صيغةٍ لهذا الفحص لم تكن تستطيع أن تفشل**: قارنتْ
   `buildCtx(بلا تثبيت)` بـ`buildCtx(tfPin: null)` — وهما يسلكان
   المسار الافتراضي **نفسه**، فأيُّ انزلاقٍ فيه يصيب الطرفين معاً
   ويُلغي الفرق. أُثبت ذلك بكسر تفضيل `sqTf` عمداً: مرّ الفحص.

   فالمقارنة الصحيحة على **التفضيلات المعلنة** لا على طرفين متطابقين:
   كلُّ دالّة اختيارٍ تُسأل مباشرةً عمّا تعيده بلا تثبيت، والقيم
   المتوقَّعة مكتوبةٌ هنا صراحةً. وهي نفس قاعدة «حارسٌ يمرّ دون أن
   تُختبر الحالةُ التي بُني لها حارسٌ مفترَض لا محروس».
   ===================================================================== */
t("تفضيلاتُ الفريم الافتراضية لم تتغيّر بإضافة التثبيت", () => {
  // سياقٌ مصطنع فيه الفريمات الأربعة كلُّها متاحة — فالاختيار تفضيلٌ
  // خالص لا نتيجةَ نقصٍ في البيانات
  const bars = Array.from({ length: 60 }, (_, i) => ({ t: i * 900e3, o: 1, h: 1, l: 1, c: 1, v: 1 }));
  const all = { "15m": bars, "1h": bars, "4h": bars, "1d": bars };
  const c = { k: all, bw: all, an: { "15m": { div: { dir: 1 } }, "1h": { div: { dir: 1 } },
                                     "4h": {}, "1d": { div: { dir: 1 } } } };
  eq(S.iTf15(c), "15m", "`iTf15` تفضّل ١٥د");
  eq(S.sqTf(c), "1h", "`sqTf` تفضّل الساعة");
  eq(S.divTf(c), "1h", "`divTf` تفضّل الساعة ثم ١٥د ثم اليومي");
  // والسقوطُ عند النقص يبقى بترتيبه
  eq(S.iTf15({ k: { "1h": bars, "1d": bars }, bw: {}, an: {} }), "1h", "١٥د غائب ⇒ الساعة");
  eq(S.sqTf({ k: {}, bw: { "4h": bars, "1d": bars }, an: {} }), "4h", "الساعة غائبة ⇒ ٤ ساعات");
  eq(S.divTf({ k: {}, bw: {}, an: { "15m": { div: { dir: 1 } } } }), "15m", "التباعد حيث يوجد");
  eq(S.divTf({ k: {}, bw: {}, an: { "1h": {} } }), null, "ولا تباعد ⇒ null لا فريمٌ مُفترَض");
  // والفريم الأعلى/الأدنى سلّمٌ لا قفزة
  eq(S.TF_UP, { "15m": "1h", "1h": "4h", "4h": "1d", "1d": null }, "سلّم الأعلى");
  eq(S.TF_DN, { "1d": "4h", "4h": "1h", "1h": "15m", "15m": null }, "وسلّم الأدنى");
});

t("والتثبيت يُطاع فعلاً حين تتوفّر بياناته", () => {
  const bars = Array.from({ length: 60 }, (_, i) => ({ t: i * 900e3, o: 1, h: 1, l: 1, c: 1, v: 1 }));
  const all = { "15m": bars, "1h": bars, "4h": bars, "1d": bars };
  const mk = (tf) => ({ k: all, bw: all, tfPin: tf,
                        an: { "15m": { div: { dir: 1 } }, "1h": { div: { dir: 1 } },
                              "4h": { div: { dir: 1 } }, "1d": { div: { dir: 1 } } } });
  for (const tf of ["15m", "1h", "4h", "1d"]) {
    eq(S.iTf15(mk(tf)), tf, `iTf15 مثبَّتة على ${tf}`);
    eq(S.sqTf(mk(tf)), tf, `sqTf مثبَّتة على ${tf}`);
    eq(S.divTf(mk(tf)), tf, `divTf مثبَّتة على ${tf}`);
  }
  // وفريمٌ لا بياناتٍ له يعيد `null` ولا يسقط إلى تفضيلٍ آخر: السقوط
  // الصامت يعطي عموداً عنوانه فريمٌ وأرقامُه من غيره
  const bare = { k: { "1d": bars }, bw: { "1d": bars }, an: {}, tfPin: "1h" };
  eq(S.iTf15(bare), null, "لا سقوط صامت عند التثبيت");
  eq(S.sqTf(bare), null, "ولا في بولنجر");
});
...
t("وما لا يقبل التثبيت يُعلَن `off` لا محايداً", () => {
  ...
  /* استراتيجيتا الجلسة والحكمُ العابر للفريمات لا تُقاس على فريمٍ
     مثبَّت — و«متعذّرة» ليست «محايدة»: الأولى تخرج من المقام، والثانية
     تخفض كلَّ نسبةٍ بلا سبب. */
  for (const tf of ["1h", "4h", "1d"]) {
    const c = S.buildCtx({ rec, row, now, sess, tfPin: tf });
    for (const id of ["orb", "vwapRec", "tfAlign"]) {
      const r = S.evalStrategy(S.STRAT_BY_ID[id], c);
      ok(r.off, `${id}@${tf} يجب أن يُعلن off`);
      ok(!r.dir, `${id}@${tf} لا يجوز أن يعطي جهة`);
    }
  }
  // و`pbTrend` يحتاج فريماً أعلى — فلا يعمل على اليوميّ (لا أسبوعيّ عندنا)
  const cD = S.buildCtx({ rec, row, now, sess, tfPin: "1d" });
  ok(S.evalStrategy(S.STRAT_BY_ID.pbTrend, cD).off, "pbTrend@1d بلا اتجاهٍ أمّ");
  // وبوابةُ «موافقة الاتجاه اليومي» تسقط عند التثبيت على اليوميّ — دائرية
  const r = S.evalStrategy(S.STRAT_BY_ID.momo, cD);
  if (r.dir && r.g)
    ok(!r.g.some(x => x[0] === "dayTrend"), "dayTrend لا يجوز أن تُحسب على عمود اليوميّ");
});

/* =====================================================================
   لا فريم دون ‎15د‎ — لا في تعريفٍ ولا في ملفٍّ مخزَّن.

   المشروع موحَّدٌ على أربعة فريمات، و‎5د‎ أُزيل من كل مسار. وعودتُه
   لا تُعلن عن نفسها بخطأ: استراتيجيةٌ توسَم `tf: "5m"` تعمل، وملفُّ
   رمزٍ يحمل `an["5m"]` يُقرأ — فقط يصير الماسح يقيس سلسلةً لا تُعرض
   في أيّ شاشة، وهي بالضبط علّة «الأرشيف يقيس شرطاً غير الذي يُعرض».

   والفحص على شقّين لأن للعلّة مصدرين: تعريفٌ في الشيفرة، وبقايا في
   القرص من قبل الإزالة.
   ===================================================================== */
t("لا فريم دون ١٥د في تعريفٍ ولا في ملفٍّ مخزَّن", () => {
  const SMALL = ["1m", "2m", "3m", "5m", "10m"];
  for (const st of S.STRATEGIES)
    ok(!SMALL.includes(st.tf), `${st.id} موسومٌ بفريمٍ صغير: ${st.tf}`);

  /* والشقّ الثاني: بقايا القرص — **على الكون كاملاً لا على عيّنة**.

     أوّل صيغةٍ لهذا الفحص أخذت أربعين ملفاً، ومرّت. والسبب أن ترتيب
     المجلّد أبجديّ وأوائلُه كلُّها كريبتو (`0G-USD`…) ولم تحمل ‎5د‎
     أصلاً — بينما أربعة ملفّاتٍ أبعد في الترتيب كانت تحمله. عيّنةٌ
     تختار نفسها بالأبجدية ليست عيّنة.

     والكلفة تُدفع بمسحٍ نصّي قبل التحليل: `includes` على المحتوى أرخص
     من `JSON.parse` بمراتب، ولا يُحلَّل إلا الملفّ المشبوه.

     **والمقام هو الكون الحيّ لا المجلّد**: فيه ملفّاتٌ يتيمة لرموزٍ
     خرجت من `symbols.json` (رموز الكريبتو بأسماء ياهو قبل الانتقال
     إلى Binance مثلاً) — لا تُعاد كتابتها أبداً فتبقى على بنيةِ يومِ
  ...
  const dirty = []; let n = 0, orphan = 0;
  for (const f of files) {
    if (!live.has(f.slice(0, -5))) { orphan++; continue; }
    n++;
    const txt = fs.readFileSync(path.join(dir, f), "utf8");
    if (!SMALL.some(s => txt.includes(`"${s}"`))) continue;
    const rec = JSON.parse(txt);
    for (const s of SMALL)
      for (const box of ["an", "tf", "tfx", "anx"])
        if (rec[box] && rec[box][s]) dirty.push(`${f}:${box}["${s}"]`);
  }
  ok(!dirty.length, `بقايا فريمٍ صغير في ${dirty.length} موضعاً — أوّلها ${dirty.slice(0, 3).join("، ")}`);
  console.log(`      (${n} ملفّ رمز حيّ فُحص${orphan ? ` · ${orphan} يتيمٌ خارج الكون استُثني` : ""})`);
});
...
/* =====================================================================
   هيستريسس الاتجاه — ومعه إثباتُ أن الفحص يُسقط نفسه.

   فحصٌ يمرّ مهما كانت المدخلات ليس فحصاً: الاختبار الأخير يعيد سلوك
   ما قبل الإصلاح (`need = 1`، أي بلا إمساك) ويشترط أن يُرصد مختلفاً.
   ===================================================================== */
t("الانقلاب لا يُنشر حتى يصمد ثلاث شمعاتٍ مغلقة", () => {
  const run = (seq, need) => {              // seq: [جهة, ختم الشمعة]
    let H = {}, out = [];
    for (const [d, b] of seq) { const r = S.dirHold(d, H, b, need); H = r.hold; out.push(r.dir); }
    return out;
  };
  const B = 900e3, t0 = 1_700_000_000_000;
  eq(run([[1, t0]]).join(), "1", "أوّل جهةٍ تُنشر فوراً — لا مرساة تُخالَف");
  eq(run([[1, t0], [1, t0 + B], [1, t0 + 2 * B]]).join(), "1,1,1", "الموافقة لا تُمسَك");
  eq(run([[1, t0], [-1, t0 + B], [-1, t0 + 2 * B]]).join(), "1,1,1",
     "شمعتان لا تكفيان — تبقى الجهة السابقة");
  eq(run([[1, t0], [-1, t0 + B], [-1, t0 + 2 * B], [-1, t0 + 3 * B]]).join(), "1,1,1,-1",
     "الثالثة تنشر الانقلاب");
  eq(run([[1, t0], [-1, t0 + B], [-1, t0 + B], [-1, t0 + B], [-1, t0 + B]]).join(), "1,1,1,1,1",
     "تكرارُ الدورات على نفس الشمعة لا يُعدّ — العدّ بالشمعات لا بالنداءات");
});

t("مرساةُ الاتجاه تنجو من السكون فلا يلتفّ الانقلاب عليها", () => {
  const B = 900e3, t0 = 1_700_000_000_000;
  let H = S.dirHold(1, {}, t0).hold;
  const q = S.dirHold(0, H, t0 + B);
  eq(q.dir, 0, "السكون يُنشر صامتاً لا بجهةٍ ملفّقة");
  eq(q.hold.ld, 1, "والمرساة تبقى");
  H = q.hold;
  const a = S.dirHold(-1, H, t0 + 2 * B); H = a.hold;
  eq(a.dir, 1, "الانقلاب بعد السكون يُقاس على الجهة السابقة لا على الصفر");
  ok(a.pend && a.pend.n === 1 && a.pend.d === -1, "ويُعلَن التربّص بجهته وعدده");
  const b = S.dirHold(-1, H, t0 + 3 * B); H = b.hold;
  eq(S.dirHold(-1, H, t0 + 4 * B).dir, -1, "ويمرّ بعد ثلاث شمعات");
});

t("عودةُ الاتجاه الأصلي تُلغي التربّص", () => {
  const B = 900e3, t0 = 1_700_000_000_000;
  let H = S.dirHold(1, {}, t0).hold;
  H = S.dirHold(-1, H, t0 + B).hold;
  ok(H.pd === -1 && H.pn === 1, "تربّصٌ قائم");
  const r = S.dirHold(1, H, t0 + 2 * B);
  eq(r.pend, null, "لا تربّص بعد العودة");
  eq(r.hold.pd, 0, "والعدّاد يُصفَّر فلا يتراكم عبر محاولتين");
});

t("بلا ختمِ شمعةٍ لا إمساك — فلا يتجمّد اتجاهٌ إلى الأبد", () => {
  const H = { ld: 1, pd: 0, pn: 0, pb: 0 };
  eq(S.dirHold(-1, H, null).dir, -1, "ختمٌ مفقود: تُنشر جهة side كما قبل الآلية");
  eq(S.dirHold(-1, H, undefined).dir, -1, "وكذلك غير المعرَّف");
  eq(S.dirHold(-1, H, NaN).dir, -1, "وكذلك NaN");
  eq(S.dirHold(-1, H, 1_700_000_000_000).dir, 1, "ومع ختمٍ صالح يُمسَك");
  /* الحارس يُثبَت بنقيضه: لولاه لبقي العدّاد على واحدٍ أبداً */
  let G = H, last = 0;
  for (let i = 0; i < 12; i++) { const r = S.dirHold(-1, G, null); G = r.hold; last = r.dir; }
  eq(last, -1, "اثنتا عشرة دورةً بلا ختم لا تُبقيه مُمسَكاً");
});

t("الفحص يُسقط نفسه لو أُعيد سلوكُ ما قبل الإصلاح", () => {
  const B = 900e3, t0 = 1_700_000_000_000;
  const run = (need) => {
    let H = S.dirHold(1, {}, t0).hold;
    return S.dirHold(-1, H, t0 + B, need).dir;
  };
  eq(run(S.DIR_HOLD), 1, "بالهيستريسس: يبقى على جهته");
  eq(run(1), -1, "وبلا هيستريسس (need=1): ينقلب فوراً — فالفرق مرصود");
  ok(S.DIR_HOLD >= 2, "وعتبةٌ أقلّ من شمعتين تُلغي الآلية عملياً");
});
