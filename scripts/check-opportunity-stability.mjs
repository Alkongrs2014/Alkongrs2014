...
t("CONFIRMED يتحرّك حين تتغيّر الشمعة المغلقة فعلاً", () => {
  const brk = S.STRAT_BY_ID.brk;
  /* =====================================================================
     **شمعةُ الاختراق يجب أن تكون في الماضي، مغلقةً فعلاً.**

     كان هذا المُثبِّت يبني سلسلةً تنتهي عند الشمعة **الجارية** ثم
     يُلحق بها شمعتين عند `+15د` و`+30د` — أي **في المستقبل** — ويتوقّع
     أن تُقرأ مغلقةً. وكان ينجح لأن `closedBars` تحذف شمعةً واحدة:
     تُحذف الأبعد فتبقى الأقرب «مغلقة» وهي لم تقع بعد.

     أي أن الاختبار كان أخضر **بسبب العلّة التي يفحصها**. وبعد أن صارت
     الدالّة تحذف كلَّ ما لم يُغلق، انكشف المُثبِّت.

     فالمرساة الآن ثلاثُ شمعاتٍ إلى الوراء: الأساس ينتهي عند
     `anchor − 3` والاختراق `anchor − 2` وتاليه `anchor − 1` — وكلُّها
     مغلقةٌ بالفعل، والاختراق يقع في موضع `k[n-2]` كما يقتضي اصطلاح
     `donch`/`volRatio`/`follow`. */
  const anchor = Math.floor(Date.now() / 900000) * 900000;
  const kFlat = flatCandles(30, 100, 0.05, anchor - 32 * 900000);
  ...
  const kBreak = kFlat.concat([
    { t: anchor - 2 * 900000, o: 200, h: 201, l: 199, c: 200, v: 100000 },
    { t: anchor - 1 * 900000, o: 200, h: 201, l: 199, c: 200, v: 100000 }
  ]);
