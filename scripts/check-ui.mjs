...
t("لا تعارض أسماء بين index.html والملفات المشتركة", () => {
  const decl = (src) => new Set([...src.matchAll(/^(?:function|const|let|var)\s+([A-Za-z_$][\w$]*)/gm)]
    .map(m => m[1]));
  /* =====================================================================
     والصفحة مع **نفسها** أولاً — ثغرةٌ بقيت مفتوحة حتى وقعتُ فيها.

     كان الفحص يقارن الصفحة بالملفات المشتركة ويقارن المشتركة ببعضها،
     ولا يقارن الصفحة بنفسها. فكتبتُ `function renderEarnings` جديدة
     وفي الصفحة واحدةٌ بنفس الاسم منذ شهور — و`function` يرفع تعريفه،
     فالأخير يغلب **بلا خطأ ولا تحذير**: البطاقة الجديدة لا تُرسم أبداً،
     والقديمة تُنادى من مكانٍ لم يقصدها. نفس عائلة `pctRank` بالضبط،
     والفارق الوحيد أن الملفّ واحد.

     و`const`/`let` المكرّران يرميان خطأً فيُسقطان الشيفرة كلَّها —
     أوضح، لكنه أيضاً ما يجب ألّا يصل المتصفّح. */
  const dupInPage = (() => {
    const seen = new Map(), dups = [];
    for (const m of inline.join("\n").matchAll(/^(?:function|const|let|var)\s+([A-Za-z_$][\w$]*)/gm)) {
      const n = m[1];
      // `var` المكرّر داخل نفس النطاق شائعٌ ومشروع في شيفرة قديمة؛
      // المقصود هنا التعريفات العليا التي تتنافس على نفس الاسم
      if (seen.has(n)) dups.push(n); else seen.set(n, true);
    }
    return [...new Set(dups)];
  })();
  if (dupInPage.length)
    throw new Error("اسمٌ معرَّف مرتين داخل الصفحة نفسها: " + dupInPage.join("، "));
